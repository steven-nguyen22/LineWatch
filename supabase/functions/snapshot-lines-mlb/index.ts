// snapshot-lines-mlb
//
// Captures consensus run-line spreads AND consensus player-prop lines
// (batter hits, batter home runs, pitcher strikeouts) for MLB games
// starting in the next 25-35 minutes. Runs every 5 minutes via pg_cron —
// each game gets caught by at least one tick of this 10-minute window.
//
...
// NULL. The companion `fetch-mlb-game-results` function fills those in
// once the box score is final.
//
// Why ESPN event ID for game_id (not Odds API event ID): the post-game
// grader pulls from ESPN, so using ESPN's ID as the canonical key makes
// the UPDATE join trivial. We resolve ESPN's ID at snapshot time by
// hitting ESPN's daily scoreboard and matching on team names.
//
// Player-ID resolution (player props):
//   `mlb_players` is the canonical (player_name, espn_id) lookup, populated
//   upstream by the `fetch-mlb-assets` function (which scans ESPN rosters
//   for all 30 teams). Mirrors the NBA pattern with `nba_players`. Players
//   missing from `mlb_players` are silently skipped here — the asset
//   pipeline handles backfill.
...
// stored in `mlb_players.player_name`.
...
Deno.serve(async (req) => {
  ...
  async function getScoreboardEvents(dateStr: string): Promise<ESPNScoreboardEvent[]> {
    ...
    try {
      const res = await espnFetch(url, supabase, "snapshot-lines-mlb");
  ...
  // ---------------------------------------------------------------------------
  // Pre-load MLB team + player ID lookups (one query each, cached for run)
  // ---------------------------------------------------------------------------
  const [{ data: teamRows }, { data: playerRows }] = await Promise.all([
    supabase.from("mlb_teams").select("team_name, espn_id"),
    supabase.from("mlb_players").select("player_name, espn_id, team_name"),
  ]);
  ...
  // and write rows to `team_game_results` + `player_game_results`.
  // ---------------------------------------------------------------------------
  let teamInserts = 0;
  let playerInserts = 0;
  ...
  for (const event of dueEvents) {
    ...
    // ---- PLAYER PROPS (hits, strikeouts, home runs) -----------------------
    ...
    for (const bm of props.bookmakers ?? []) {
      for (const market of bm.markets ?? []) {
        ...
        for (const o of market.outcomes ?? []) {
          ...
          // boolean-style markets (home runs occasionally), "Over" for the rest.
    ...
    for (const [normKey, propMap] of buckets) {
      ...
      if (!lookup) continue; // unknown player (not in mlb_players — let fetch-mlb-assets backfill)
