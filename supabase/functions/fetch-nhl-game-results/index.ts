...
// start time — both team spreads and player props (goals, shots on goal,
// hockey points = G+A).
//
...
//
// OT / Shootout (team rows): ESPN scoreboard reflects NHL's convention
// (SO winner score boosted by exactly 1), so the formula above grades the
// puck line correctly without any special-casing.
//
// Box-score parsing for player props:
//   For each completed game we hit ESPN's `summary` endpoint and walk
//   `boxscore.players[].statistics[]`. NHL has skater stats including
//   `G` (goals), `A` (assists), `SOG` or `S` (shots on goal). The stat
//   key labels vary slightly across ESPN responses, so we look up multiple
//   variants. Hockey points (player_points prop) are computed as G + A
//   directly — robust against ESPN sometimes omitting a "PTS" column.
 ...
 * index, or -1 if none match. ESPN occasionally uses different labels
 * across responses (e.g. "S" vs "SOG" for shots on goal).
 */
...
Deno.serve(async (req) => {
  ...
  for (const event of completed) {
    try {
      ...
      // ---- PLAYER RESULTS (per-skater goals / shots / G+A points) ---------
      // NHL box-score has skater + goalie groups per team. All three of our
      // props are skater stats. Goalies have their own group with different
      ...
      for (const team of boxTeams) {
        for (const statgroup of team.statistics ?? []) {
          ...
          for (const a of statgroup.athletes ?? []) {
            ...
            const goals = parseStat(a.stats?.[goalsIdx]);
            const assists = parseStat(a.stats?.[assistsIdx]);
            const shots = shotsIdx >= 0 ? parseStat(a.stats?.[shotsIdx]) : null;

            // Map prop_type → actual value (computed for hockey points).
            const actualByProp: Record<string, number | null> = {
              player_goals: goals,
              player_shots_on_goal: shots,
              player_points: goals !== null && assists !== null ? goals + assists : null,
            };
